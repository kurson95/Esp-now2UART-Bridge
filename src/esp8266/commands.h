...
void printSystemInfo()
{
  ...
  info += "Flash chip size: " + String(ESP.getFlashChipSize()) + " bytes\n";
  info += "Flash real size: " + String(ESP.getFlashChipRealSize()) + " bytes\n";
  ...
  info += "Chip ID: 0x" + String(ESP.getChipId(), HEX) + endLine;
...
void handleCommand(String input)
{
  ...
  case ADDRECV:
  {
    ...
    else
    {
      ...
      {
        logger.log(LOG_INFO, "PEER ADD ERROR");
        return;
      }
      setENC(1);
    }
  ...
  case SEND:
  {
    ...
    {
      esp_now_send(peerAddress, (uint8_t *)subcmd.c_str(), subcmd.length());
